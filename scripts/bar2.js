...
function updateBars() {
	...
	dataJoin.enter().append("rect")//null set
		.attr("x", function(d,i){return barConfig.xScale * i;}) //select some of them and pass them down 
		.attr("y", function(d,i){return barConfig.chartHeight - barConfig.yScale * d.v;})
		.attr("width", function(d,i){return barConfig.barWidth;})
		.attr("height", function(d,i) {return 0;})// //{return barConfig.yScale * d.v;})
		;
	// The "update" set now includes the "enter" set // you take everything 

	// A transition is applied to smootly change the data
	dataJoin.attr("x", function(d,i){return barConfig.xScale * i;})//real selection of the rectangle
